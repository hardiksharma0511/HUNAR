...
const CartPage = () => {
  ...
  const handleQuantityChange = async (productId: string, newQty: number, stock: number) => {
    if (newQty < 1) return;
    if (newQty > stock) {
      setItemErrors((prev) => ({ ...prev, [productId]: `Only ${stock} in stock` }));
      setTimeout(() => setItemErrors((prev) => ({ ...prev, [productId]: "" })), 2500);
      return;
    }
    ...
    try {
      ...
      setItemErrors((prev) => ({ ...prev, [productId]: err.response?.data?.message || "Could not update quantity" }));
    }
  };
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-3 mt-2">
                    ...
                    <button
                      onClick={() => handleQuantityChange(item.product._id, item.quantity + 1, item.product.stock)}
                      disabled={atMax}
                      className="w-7 h-7 rounded-full border border-terracotta/30 flex items-center justify-center disabled:opacity-30 disabled:cursor-not-allowed"
                    >
                      <Plus className="w-3.5 h-3.5" />
                    </button>
                  </div>
                  {itemErrors[item.product._id] && (
                    <p className="text-xs text-red-600 mt-1">{itemErrors[item.product._id]}</p>
                  )}
                </div>
